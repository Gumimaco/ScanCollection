...
export const BrowsePage: React.FC<BrowsePageProps> = ({}) => {
    ...
    return (
        <div>
            ...
            <div className="">
                <div>
                    <div className="flex relative ml-4 mt-6 gap-1 justify-center items-center">
                        <input className={`${toDisplayFilter === 'genres' ? 'bg-amber-500' : 'bg-pearl'} rounded px-1 text-black hover:bg-amber-500 ease-in-out duration-150 `} type="button" id="button-genres" value="Genres" onClick={(e) => toggle_show_button('genres',e)}/>
                        <input className={`${toDisplayFilter === 'sort' ? 'bg-amber-500' : 'bg-pearl'}  rounded px-1 text-black hover:bg-amber-500 ease-in-out duration-150 `} type="button" id="button-sort" value="Sort By" onClick={(e) => toggle_show_button('sort',e)}/>
                        <input className={`${toDisplayFilter === 'source' ? 'bg-amber-500' : 'bg-pearl'}  rounded px-1 text-black hover:bg-amber-500 ease-in-out duration-150`} type="button" id="button-source" value="Source" onClick={(e) => toggle_show_button('source',e)}/>
                        <input className={`${toDisplayFilter === 'status' ? 'bg-amber-500' : 'bg-pearl'}  rounded px-1 text-black hover:bg-amber-500 ease-in-out duration-150`} type="button" id="button-status" value="Status" onClick={(e) => toggle_show_button('status',e)}/>
                    </div>
                    <div className="flex justify-center mt-2">
                        <input className="bg-pearl rounded px-1 text-black hover:bg-amber-500 ease-in-out duration-150" type="submit" id="button-search" value="Search" onClick={createURLandJUMP}/>
                    </div>
                </div>
